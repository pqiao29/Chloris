...
plot_inout <- function(input, cluster_labels = NULL,
    ...
    if(!is.null(cluster_labels)){
        ...
        for(k in 1:K){
            ...
            gg_ret <- gg_ret + annotate("rect", xmin = cluster_idx[1] - 0.5, xmax = cluster_idx[length(cluster_idx)] + 0.5,
        ...
        if(length(cluster_labels) > 1){
            for(t in 2:length(cluster_labels)){
                ...
                for(i in 1:N){
                    ...
                    gg_ret <- gg_ret + annotate("rect", xmin = i - 0.5, xmax = i + 0.5,
                                                ymin = y_low, ymax =  y_high,
        ...
        if(!is.null(CN_states)){
            ...
            for(k in 1:K){
                cluster_idx <- which(I_sorted == k)
                gg_ret <- gg_ret + annotate("rect", xmin = x_state_left + (k - 1)*x_state_step - 0.5,
